...
[Watch the 56-second silent video](media/yoshida-silent-demo.mp4) · [Download timed text (WebVTT)](media/yoshida-silent-demo.vtt)

This alternate #45 deliverable uses animated text and a live-style interface instead of a participant voice or a new provider call. It can be shared while #40 remains open, as a **visual explanation of the previously verified local flow**. Its moving caller, request-detail and cleaner-review screens are illustrative reconstruction, visibly labelled throughout. The shown caller receipt and confirmed Upcoming state are real app captures made from a disposable copy of the verified #40 fictional data. Neither screenshot is contemporaneous footage of the call or owner click. The film has no audio track and does not claim to demonstrate speech quality, a new live call, a fresh booking, or the unfinished strict #40 checks.

The on-screen subtitles and WebVTT tell the same story: a fictional caller starts in the German browser simulator; Yoshida gathers the address, time and service; the caller approves a read-back; a **tentative** request is saved; the cleaner reviews and confirms it in English; and the confirmed state is shown from the verified data copy. The read-back scene is a summary, not a quotation or transcript of spoken words. The saved receipt ID shown in the video is `zhwevpk3x2cayoh`, which matches [#40's independently checked call A](integrated-acceptance-verification.md#fresh-live-german-browser-run). Its owner decision was Confirm. The video source and frame timing are in [`scripts/render_showcase_video.py`](../scripts/render_showcase_video.py). To regenerate, install [`scripts/video-requirements.txt`](../scripts/video-requirements.txt) and run `python3 scripts/render_showcase_video.py` with `ffmpeg` on the path.
